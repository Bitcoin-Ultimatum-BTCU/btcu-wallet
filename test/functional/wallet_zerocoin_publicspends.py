...
class ZerocoinSpendTest(BtcuTestFramework):
    ...
    def run_test(self):
        ...
        def check_balances(denom, zbtcu_bal, btcu_bal):
            zbtcu_bal -= denom
            assert_equal(self.nodes[2].getzerocoinbalance()['Total'], zbtcu_bal)
            btcu_bal += denom
            wi = self.nodes[2].getwalletinfo()
            assert_equal(wi['balance'] + wi['immature_balance'], btcu_bal)
            return zbtcu_bal, btcu_bal
        ...
        zbtcu_balance = self.nodes[2].getzerocoinbalance()['Total']
        assert_equal(balance, DecimalAmt(13833.92))
        assert_equal(zbtcu_balance, 6666)
        ...
        zbtcu_balance, balance = check_balances(denom_2, zbtcu_balance, balance)
        ...
        block_time = stake_4_blocks(block_time)
        self.check_tx_in_chain(0, txid)
        zbtcu_balance, balance = check_balances(denom_3, zbtcu_balance, balance)
        ...
        _, _ = check_balances(denom_1, zbtcu_balance, balance)
        self.log.info("--> VALID PUBLIC COIN SPEND (v3) PASSED")
