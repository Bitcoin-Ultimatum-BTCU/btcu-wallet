...
*Setup instructions for a gitian build of BTCU Core using a VM or physical system.*

Gitian is the deterministic build process that is used to build the BTCU
...
to the BTCU GitHub release page.
...
- [Building BTCU Core](#building-btcu-core)
...
```bash
gpg --output ${VERSION}-linux/${NAME}/btcu-linux-${VERSION%\.*}-build.assert.sig --detach-sign ${VERSION}-linux/$NAME/btcu-linux-${VERSION%\.*}-build.assert
gpg --output ${VERSION}-osx-unsigned/$NAME/btcu-osx-${VERSION%\.*}-build.assert.sig --detach-sign ${VERSION}-osx-unsigned/$NAME/btcu-osx-${VERSION%\.*}-build.assert
gpg --output ${VERSION}-win-unsigned/$NAME/btcu-win-${VERSION%\.*}-build.assert.sig --detach-sign ${VERSION}-win-unsigned/$NAME/btcu-win-${VERSION%\.*}-build.assert
```
...
You can also mail the files to Fuzzbawls (fuzzbawls@btcu.io) and he will commit them.

```bash
gpg --detach-sign ${VERSION}-linux/${NAME}/btcu-linux-*-build.assert
gpg --detach-sign ${VERSION}-win-unsigned/${NAME}/btcu-win-*-build.assert
gpg --detach-sign ${VERSION}-osx-unsigned/${NAME}/btcu-osx-*-build.assert
```
