...
bool initStakeInput(const CBlock& block, std::unique_ptr<CStakeInput>& stake, int nPreviousBlockHeight) {
    ...
    if (txin.IsZerocoinSpend()) {
        ...
        const libzerocoin::CoinDenomination denom = libzerocoin::AmountToZerocoinDenomination(zBTCU->GetValue());
        if (ParseAccChecksum(chainActive[cpHeight]->nAccumulatorCheckpoint, denom) != zBTCU->GetChecksum())
