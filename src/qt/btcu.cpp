...
#include "qt/btcu/splash.h"
#include "qt/btcu/welcomecontentwidget.h"
...
{
    return QCoreApplication::translate("btcu-core", psz).toStdString();
}
...
/** Class encapsulating BTCU Core startup and shutdown.
...
public:
    ...
    /// Get window identifier of QMainWindow (BTCUGUI)
...
{
    window = new BTCUGUI(networkStyle, 0);
...
void BitcoinApplication::initializeResult(int retval)
{
    ...
    if (retval) {
        ...
        if (pwalletMain) {
            ...
            window->addWallet(BTCUGUI::DEFAULT_WALLET, walletModel);
            window->setCurrentWallet(BTCUGUI::DEFAULT_WALLET);
...
{
    QMessageBox::critical(0, "Runaway exception", BTCUGUI::tr("A fatal error occurred. BTCU can no longer continue safely and will quit.") + QString("\n\n") + message);
    ::exit(1);
}
...
/// 2. Basic Qt initialization (not dependent on parameters or configuration)
    Q_INIT_RESOURCE(btcu_locale);
    Q_INIT_RESOURCE(btcu);
    ...
    /// 6. Determine availability of data directory and parse btcu.conf
    /// - Do not call GetDataDir(true) before this step finishes
    if (!boost::filesystem::is_directory(GetDataDir(false))) {
        QMessageBox::critical(0, QObject::tr("BTCU Core"),
    ...
    if (!SelectParamsFromCommandLine()) {
        QMessageBox::critical(0, QObject::tr("BTCU Core"), QObject::tr("Error: Invalid combination of -regtest and -testnet."));
        return 1;
    }
#ifdef ENABLE_WALLET
    ...
    // btcu: links repeatedly have their payment requests routed to this process:
...
#if defined(Q_OS_WIN)
        WinShutdownMonitor::registerShutdownBlockReason(QObject::tr("BTCU Core didn't yet exit safely..."), (HWND)app.getMainWinId());
#endif
        app.exec();
        app.requestShutdown();
        app.exec();
