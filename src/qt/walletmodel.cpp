...
bool WalletModel::isLeasingNetworkelyEnabled() const {
    return sporkManager.IsSporkActive(SPORK_1017_LEASING_ENFORCEMENT);
}
...
CAmount WalletModel::getMinLeasingAmount() const
{
    return Params().GetMinLeasingAmount();
}
...
bool WalletModel::isLeasing() const
{
    // TODO: Complete me..
    return false;
}
...
WalletModel::SendCoinsReturn WalletModel::prepareTransaction(WalletModelTransaction& transaction, const CCoinControl* coinControl)
{
    ...
    Q_FOREACH (const SendCoinsRecipient& rcp, recipients) {
        if (rcp.paymentRequest.IsInitialized()) { // PaymentRequest...
            ...
            total += subtotal;
        } else { // User-entered btcu address / amount:
...

    {
        ...
        if (recipients[0].useSwiftTX && total > sporkManager.GetSporkValue(SPORK_5_MAX_VALUE) * COIN) {
            Q_EMIT message(tr("Send Coins"), tr("SwiftX doesn't support sending values that high yet. Transactions are currently limited to %1 BTCU.").arg(sporkManager.GetSporkValue(SPORK_5_MAX_VALUE)),
...
WalletModel::SendCoinsReturn WalletModel::sendCoins(WalletModelTransaction& transaction)
{
    ...
    if(!CheckTransaction(*transaction.getTransaction(), true, true, state, true, fColdStakingActive, fLeasingActive)){
...

    {
        ...
        Q_FOREACH (const SendCoinsRecipient& rcp, recipients) {
            ...
            } else if (!rcp.message.isEmpty()) // Message from normal btcu:URI (btcu:XyZ...?message=example)
            {
...
bool WalletModel::convertBackZbtcu(
        CAmount value,
        std::vector<CZerocoinMint> &vMintsSelected,
        CZerocoinSpendReceipt &receipt
){
...
static void NotifyzBTCUReset(WalletModel* walletmodel)
{
    qDebug() << "NotifyzBTCUReset";
...
void WalletModel::subscribeToCoreSignals()
{
    ...
    wallet->NotifyzBTCUReset.connect(boost::bind(NotifyzBTCUReset, this));
...
void WalletModel::unsubscribeFromCoreSignals()
{
    ...
    wallet->NotifyzBTCUReset.disconnect(boost::bind(NotifyzBTCUReset, this));
...
{
    setMints.clear();
    setMints = pwalletMain->zbtcuTracker->ListMints(fUnusedOnly, fMaturedOnly, fUpdateStatus, fWrongSeed);
}
