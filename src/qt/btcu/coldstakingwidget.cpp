...
#include "qt/btcu/forms/ui_coldstakingwidget.h"
...
ColdStakingWidget::ColdStakingWidget(BTCUGUI* parent) :
    PWidget(parent),
    ...
    isLoading(false)
{
    ...
    /* Subtitle */
    ui->labelSubtitle1->setText(tr("You can delegate your BTCUs, letting a hot node (24/7 online node)\nstake on your behalf, while you keep the keys securely offline."));
    ...
    ui->labelSubtitle2->setText(tr("Accept BTCU delegation / Delegate BTCU"));
    ...
    ui->btnCoinControl->setSubTitleClassAndText("text-subtitle", "Select BTCU outputs to delegate.");
