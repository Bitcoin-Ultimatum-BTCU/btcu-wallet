...
void BTCUGUI::connectActions() {
    ...
    connect(addressesWidget, &AddressesWidget::showHide, this, &BTCUGUI::showHide);
    connect(privacyWidget, &PrivacyWidget::showHide, this, &BTCUGUI::showHide);
    connect(masterNodesWidget, &MasterNodesWidget::showHide, this, &BTCUGUI::showHide);
...
void BTCUGUI::createTrayIcon(const NetworkStyle* networkStyle) {
#ifndef Q_OS_MAC
    trayIcon = new QSystemTrayIcon(this);
    QString toolTip = tr("BTCU Core client") + " " + networkStyle->getTitleAddText();
...
void BTCUGUI::handleRestart(QStringList args){
    if (!ShutdownRequested())
        Q_EMIT requestedRestart(args);
}


void BTCUGUI::setClientModel(ClientModel* clientModel) {
...
void BTCUGUI::createTrayIconMenu() {
#ifndef Q_OS_MAC
...
#ifndef Q_OS_MAC
void BTCUGUI::trayIconActivated(QSystemTrayIcon::ActivationReason reason)
{
...
void BTCUGUI::changeEvent(QEvent* e)
{
...
void BTCUGUI::closeEvent(QCloseEvent* event)
{
#ifndef Q_OS_MAC // Ignored on Mac
...
void BTCUGUI::message(const QString& title, const QString& message, unsigned int style, bool* ret) {
    QString strTitle =  tr("BTCU Core"); // default title
...
bool BTCUGUI::openStandardDialog(QString title, QString body, QString okBtn, QString cancelBtn){
    ...
    if (isVisible()) {
        ...
        dialog->setWindowTitle(tr("BTCU Core"));
        dialog->adjustSize();
        dialog->raise();
        dialog->exec();
    }
...
void BTCUGUI::showNormalIfMinimized(bool fToggleHidden) {
    if (!clientModel)
        return;
...
void BTCUGUI::toggleHidden() {
    showNormalIfMinimized(true);
}

void BTCUGUI::detectShutdown() {
    if (ShutdownRequested()) {
        if (rpcConsole)
            rpcConsole->hide();
        qApp->quit();
...
void BTCUGUI::goToSend(){
    showTop(sendWidget);
}

void BTCUGUI::goToAddresses(){
    showTop(addressesWidget);
}

void BTCUGUI::goToPrivacy(){
    showTop(privacyWidget);
}

void BTCUGUI::goToMasterNodes(){
    showTop(masterNodesWidget);
}

void BTCUGUI::goToColdStaking(){
...
void BTCUGUI::changeTheme(bool isLightTheme){
...
void BTCUGUI::resizeEvent(QResizeEvent* event){
    // Parent..
...
int BTCUGUI::getNavWidth(){
    return this->navMenu->width();
}

void BTCUGUI::openFAQ(int section){
    showHide(true);
...
#ifdef ENABLE_WALLET
bool BTCUGUI::addWallet(const QString& name, WalletModel* walletModel)
{
    ...
    connect(addressesWidget, &AddressesWidget::message,this, &BTCUGUI::message);
    connect(settingsWidget, &SettingsWidget::message, this, &BTCUGUI::message);
...
bool BTCUGUI::setCurrentWallet(const QString& name) {
    // Single wallet supported.
    return true;
}

void BTCUGUI::removeAllWallets() {
    // Single wallet supported.
}

void BTCUGUI::incomingTransaction(const QString& date, int unit, const CAmount& amount, const QString& type, const QString& address) {
...
static bool ThreadSafeMessageBox(BTCUGUI* gui, const std::string& message, const std::string& caption, unsigned int style)
{
