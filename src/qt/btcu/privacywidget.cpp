...
#include "qt/btcu/forms/ui_privacywidget.h"
#include "qt/btcu/qtutils.h"
#include "guiutil.h"
#include "qt/btcu/denomgenerationdialog.h"
...
PrivacyWidget::PrivacyWidget(BTCUGUI* parent) :
    PWidget(parent),
    ui(new Ui::PrivacyWidget)
{
    ...
    /* Subtitle */
    ui->labelSubtitle1->setText(tr("Minting zBTCU anonymizes your BTCU by removing any\ntransaction history, making transactions untraceable "));
    ...
    ui->labelValueDenom1->setText("0x1 = 0 zBTCU");
    ...
    ui->labelValueDenom5->setText("0x5 = 0 zBTCU");
    ...
    ui->labelValueDenom10->setText("0x10 = 0 zBTCU");
    ...
    ui->labelValueDenom50->setText("0x50 = 0 zBTCU");
    ...
    ui->labelValueDenom100->setText("0x100 = 0 zBTCU");
    ...
    ui->labelValueDenom500->setText("0x500 = 0 zBTCU");
    ...
    ui->labelValueDenom1000->setText("0x1000 = 0 zBTCU");
    ...
    ui->labelValueDenom5000->setText("0x5000 = 0 zBTCU");
    ...
    ui->btnTotalzBTCU->setTitleClassAndText("btn-title-grey", "Total 0 zBTCU");
    ui->btnTotalzBTCU->setSubTitleClassAndText("text-subtitle", "Show denominations of zBTCU owned.");
    ui->btnTotalzBTCU->setRightIconClass("ic-arrow");

    ui->btnCoinControl->setTitleClassAndText("btn-title-grey", "Coin Control");
    ui->btnCoinControl->setSubTitleClassAndText("text-subtitle", "Select BTCU outputs to mint into zBTCU.");
    ...
    ui->btnResetZerocoin->setTitleClassAndText("btn-title-grey", "Reset Spent zBTCU");
    ui->btnResetZerocoin->setSubTitleClassAndText("text-subtitle", "Reset zerocoin database.");

    connect(ui->btnTotalzBTCU, SIGNAL(clicked()), this, SLOT(onTotalZbtcuClicked()));
...
void PrivacyWidget::onMintSelected(bool isMint){
    QString btnText;
    if(isMint){
        btnText = tr("Mint zBTCU");
        ui->btnCoinControl->setVisible(true);
        ui->labelSubtitleAmount->setText(tr("Enter amount of BTCU to mint into zBTCU"));
    }else{
        btnText = tr("Convert back to BTCU");
        ui->btnCoinControl->setVisible(false);
        ui->labelSubtitleAmount->setText(tr("Enter amount of zBTCU to convert back into BTCU"));
    }
...
void PrivacyWidget::onTotalZbtcuClicked(){
    bool isVisible = ui->layoutDenom->isVisible();
    if(!isVisible){
        ui->layoutDenom->setVisible(true);
        ui->btnTotalzBTCU->setRightIconClass("btn-dropdown", true);
    }else{
        ui->layoutDenom->setVisible(false);
        ui->btnTotalzBTCU->setRightIconClass("ic-arrow", true);
    }
}
...
void PrivacyWidget::onSendClicked(){
    ...
    if(sporkManager.IsSporkActive(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        warn(tr("Zerocoin"), tr("zBTCU is currently undergoing maintenance"));
        return;
    }
    ...
    if(!GUIUtil::requestUnlock(walletModel, AskPassphraseDialog::Context::Mint_zBTCU, true)){
        inform(tr("You need to unlock the wallet to be able to %1 zBTCU").arg(isConvert ? tr("convert") : tr("mint")));
        return;
    }
...
void PrivacyWidget::mint(CAmount value){
    ...
    if(!walletModel->mintCoins(value, CoinControlDialog::coinControl, strError)){
        ...
        // Mint succeed
        inform(tr("zBTCU minted successfully"));
        // clear
...
void PrivacyWidget::spend(CAmount value){
    ...
    if(!walletModel->convertBackZbtcu(
            value,
            selectedMints,
            receipt
    )){
        ...
        // Spend succeed
        inform(tr("zBTCU converted back to BTCU"));
        // clear
        ui->lineEditAmount->clear();
    }
}
...
void PrivacyWidget::updateDenomsSupply(){
    ...
    for (const auto& denom : libzerocoin::zerocoinDenomList) {
        ...
        strDenomStats = strUnconfirmed + QString::number(mapDenomBalances.at(denom)) + " x " +
                        ...
                        QString::number(nSumPerCoin) + " zBTCU </b>";
    ...
    ui->btnTotalzBTCU->setTitleText(tr("Total %1").arg(GUIUtil::formatBalance(matureZerocoinBalance, nDisplayUnit, true)));
}
