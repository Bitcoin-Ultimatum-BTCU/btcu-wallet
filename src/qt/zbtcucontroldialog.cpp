...
#include "ui_zbtcucontroldialog.h"
    ...
    model(0)
{
    ...
    // Title
    ui->labelTitle->setText(tr("Select zBTCU Denominations to Spend"));
    ...
    for (const CMintMeta& mint : setMints) {
        ...
        if(!fSpendable) {
            ...
            if(nConfirmations < Params().Zerocoin_MintRequiredConfirmations())
                ...
                strReason = "Your wallet is locked. Impossible to spend zBTCU.";
            else if (!mint.isSeedCorrect)
                strReason = "The zBTCU seed used to mint this zBTCU is not the same as currently hold in the wallet";
            else
