...
void OptionsModel::setDisplayDefaultOptions(QSettings& settings, bool reset){
    if (!settings.contains("nDisplayUnit") || reset)
        settings.setValue("nDisplayUnit", BitcoinUnits::BTCU);
    ...
    if (settings.contains("nAnonymizeBtcuAmount") || reset)
        SoftSetArg("-anonymizebtcuamount", settings.value("nAnonymizeBtcuAmount").toString().toStdString());
...
void OptionsModel::Reset()
{
    ...
    settings.clear();
    resetSettings = true; // Needed in btcu.cpp during shotdown to also remove the window positions
