...
/** Fees smaller than this (in ubtcu) are considered zero fee (for relaying and mining)
...
CValidatorsState g_ValidatorsState;
...
bool ContextualCheckZerocoinSpend(const CTransaction& tx, const libzerocoin::CoinSpend* spend, int nHeight, const uint256& hashBlock)
{
    ...
    if (IsSerialInBlockchain(spend->getCoinSerialNumber(), nHeightTx))
        return error("%s : zBTCU spend with serial %s is already in block %d\n", __func__,
...
{
    //Check to see if the zBTCU is properly signed
    if (nHeight >= Params().Zerocoin_Block_V2_Start()) {
        try {
            if (!spend->HasValidSignature())
                return error("%s: V2 zBTCU spend does not have a valid signature\n", __func__);
        ...
        if (spend->getSpendType() != expectedType) {
            return error("%s: trying to spend zBTCU without the correct spend type. txid=%s\n", __func__,
                         tx.GetHash().GetHex());
        }
    }
    ...
    if (!spend->HasValidSerial(Params().Zerocoin_Params(fUseV1Params)))  {
        ...
        if (!isBlockBetweenFakeSerialAttackRange(nHeight))
            return error("%s : zBTCU spend with serial %s from tx %s is not in valid range\n", __func__,
...
bool CheckZerocoinSpend(const CTransaction& tx, bool fVerifySignature, CValidationState& state, bool fFakeSerialAttack)
{
    ...
    for (const CTxIn& txin : tx.vin) {
        ...
        if (isPublicSpend) {
            ...
            if (!ZBTCUModule::parseCoinSpend(txin, tx, prevOut, publicSpend)){
        ...
        if (isPublicSpend) {
            ...
            if (!ZBTCUModule::validateInput(txin, prevOut, tx, ret)){
...

    {
        ...
        if (hasZcSpendInputs) {
            ...
            if (IsTransactionInChain(tx.GetHash(), nHeightTx))
                return state.Invalid(error("%s : zBTCU spend tx %s already in block %d",
            ...
            for (const CTxIn& txIn : tx.vin) {
                ...
                if (!txIn.IsZerocoinPublicSpend())
                    return state.Invalid(error("%s: failed for tx %s, every input must be a zcpublicspend",
                            __func__, tx.GetHash().GetHex()), REJECT_INVALID, "bad-txns-invalid-zbtcu");

                libzerocoin::ZerocoinParams* params = Params().Zerocoin_Params(false);
                PublicCoinSpend publicSpend(params);
                if (!ZBTCUModule::ParseZerocoinPublicSpend(txIn, tx, state, publicSpend)){
                    return false;
                }
                ...
                if (!CheckPublicCoinSpendVersion(publicSpend.getVersion())) {
                    return state.Invalid(error("%s : Public Zerocoin spend version %d not accepted. must be version %d.",
                            __func__, publicSpend.getVersion(), CurrentPublicCoinSpendVersion()), REJECT_INVALID, "bad-txns-invalid-zbtcu");
                }
        ...
        for (const CTxIn& in : tx.vin) {
            ...
            if (in.IsZerocoinSpend() || isPublicSpend) {
                ...
                if (isPublicSpend) {
                    ...
                    if (!ZBTCUModule::ParseZerocoinPublicSpend(in, tx, state, publicSpend)){
...
bool DisconnectBlock(CBlock& block, CValidationState& state, CBlockIndex* pindex, CCoinsViewCache& view, bool* pfClean)
{
    ...
    for (int i = block.vtx.size() - 1; i >= 0; i--) {
         ...
         * note we only undo zerocoin databasing in the following statement, value to and from BTCU
        ...
        UpdateValidatorsVotingState(tx, false);
    }
...
void RecalculateZBTCUMinted()
{
    CBlockIndex *pindex = chainActive[Params().Zerocoin_StartHeight()];
    const int chainHeight = chainActive.Height();
    uiInterface.ShowProgress(_("Recalculating minted ZBTCU..."), 0);
    while (true) {
        ...
        if (pindex->nHeight % 1000 == 0) {
            ...
            uiInterface.ShowProgress(_("Recalculating minted ZBTCU..."), percent);
        }
...
void RecalculateZBTCUSpent()
{
    CBlockIndex* pindex = chainActive[Params().Zerocoin_StartHeight()];
    const int chainHeight = chainActive.Height();
    uiInterface.ShowProgress(_("Recalculating spent ZBTCU..."), 0);
    while (true) {
        if (pindex->nHeight % 1000 == 0) {
            LogPrintf("%s : block %d...\n", __func__, pindex->nHeight);
            int percent = std::max(1, std::min(99, (int)((double)(pindex->nHeight - Params().Zerocoin_StartHeight()) * 100 / (chainHeight - Params().Zerocoin_StartHeight()))));
            uiInterface.ShowProgress(_("Recalculating spent ZBTCU..."), percent);
        }

        //Rewrite zBTCU supply
        CBlock block;
        assert(ReadBlockFromDisk(block, pindex));
...
bool RecalculateBTCUSupply(int nHeightStart)
{
    const int chainHeight = chainActive.Height();
    ...
    while (true) {
        if (pindex->nHeight % 1000 == 0) {
            ...
            uiInterface.ShowProgress(_("Recalculating BTCU supply..."), percent);
        }
...
bool UpdateZBTCUSupply(const CBlock& block, CBlockIndex* pindex, bool fJustCheck)
{
    std::list<CZerocoinMint> listMints;
    ...
    // A one-time event where only the zBTCU supply was off (due to serial duplication off-chain on main net)
...
bool ConnectBlock(const CBlock& block, CValidationState& state, CBlockIndex* pindex, CCoinsViewCache& view, bool fJustCheck, bool fAlreadyChecked)
{
    ...
    if(!block.vchValidatorSig.empty()){
        if(!CheckValidator(block, view)){
            return state.DoS(100, error("ConnectBlock() : Invalid validator"),
                             REJECT_INVALID, "validator-invalid");
        }
    }
    ...
    if (!UpdateZBTCUSupply(block, pindex, fJustCheck))
        return state.DoS(100, error("%s: Failed to calculate new zBTCU supply for block=%s height=%d", __func__,
    ...
    for(auto &tx : validatorTransactions){
        UpdateValidatorsVotingState(tx, true);
    }
    UpdateValidators(pindex);
...
bool CheckColdStakeFreeOutput(const CTransaction& tx, const int nHeight)
{
    ...
    if (outs >=3 && lastOut.scriptPubKey != tx.vout[outs-2].scriptPubKey) {
        ...
        // cold staking is active much after nPublicZCSpends so GetMasternodePayment is always 3 BTCU.
...
bool CheckBlock(const CBlock& block, CValidationState& state, bool fCheckPOW, bool fCheckMerkleRoot, bool fCheckSig)
{
    ...
    for (const CTransaction& tx : block.vtx) {
        ...
        // double check that there are no double spent zBTCU spends in this block
        if (tx.HasZerocoinSpendInputs()) {
            for (const CTxIn& txIn : tx.vin) {
                bool isPublicSpend = txIn.IsZerocoinPublicSpend();
                if (txIn.IsZerocoinSpend() || isPublicSpend) {
                    ...
                    if (std::count(vBlockSerials.begin(), vBlockSerials.end(), spend.getCoinSerialNumber()))
                        return state.DoS(100, error("%s : Double spending of zBTCU serial %s in block\n Block: %s",
...
bool CheckWork(const CBlock block, CBlockIndex* const pindexPrev)
{
    ...
    if (block.nBits != nBitsRequired) {
        // Btcu Specific reference to the block with the wrong threshold was used.
        if ((block.nTime == (uint32_t) Params().BtcuBadBlockTime()) && (block.nBits == (uint32_t) Params().BtcuBadBlocknBits())) {
            // accept BTCU block minted with incorrect proof of work threshold
            return true;
        }
...
bool AcceptBlock(CBlock& block, CValidationState& state, CBlockIndex** ppindex, CDiskBlockPos* dbp, bool fAlreadyCheckedBlock)
{
    ...
    if (isPoS) {
        ...
        // Inputs
        std::vector<CTxIn> btcuInputs;
        std::vector<CTxIn> zBTCUInputs;

        for (const CTxIn& stakeIn : stakeTxIn.vin) {
            if(stakeIn.IsZerocoinSpend()){
                zBTCUInputs.push_back(stakeIn);
            }else{
                btcuInputs.push_back(stakeIn);
            }
        }
        const bool hasBTCUInputs = !btcuInputs.empty();
        const bool hasZBTCUInputs = !zBTCUInputs.empty();
        ...
        for (const CTransaction& tx : block.vtx) {
            for (const CTxIn& in: tx.vin) {
                ...
                if(hasBTCUInputs) {
                    ...
                    for (const CTxIn& btcuIn : btcuInputs)
                        if(btcuIn.prevout == in.prevout)
        ...
        if (isBlockFromFork) {
            ...
            // Now that this loop if completed. Check if we have zBTCU inputs.
            if(hasZBTCUInputs) {
                for (const CTxIn& zPivInput : zBTCUInputs) {
                    ...
                    if (!ContextualCheckZerocoinSpendNoSerialCheck(stakeTxIn, &spend, pindex->nHeight, 0))
                        return state.DoS(100,error("%s: forked chain ContextualCheckZerocoinSpend failed for tx %s", __func__,
                                                   stakeTxIn.GetHash().GetHex()), REJECT_INVALID, "bad-txns-invalid-zbtcu");
...
bool static ProcessMessage(CNode* pfrom, std::string strCommand, CDataStream& vRecv, int64_t nTimeReceived)
{
    ...
    if (strCommand == "version") {
        ...
        // BTCU: We use certain sporks during IBD, so check to see if they are
