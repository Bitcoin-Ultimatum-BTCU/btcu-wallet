...
{
    // The wallet only allows publicSpend inputs in the same tx and not a combination between btcu and zbtcu
    for(const CTxIn& txin : vin) {
...
{
    // Should contain fee payment
    if (vin.empty() || vout.empty())
        return false;
    
    return (!validatorRegister.empty());
}

bool CTransaction::IsValidatorVote() const
{
    // Should contain fee payment
    if (vin.empty() || vout.empty())
        return false;
    
    return (!validatorVote.empty());
}
...
CAmount CTransaction::GetValueOut() const
{
    ...
    {
        // BTCU: previously MoneyRange() was called here. This has been replaced with negative check and boundary wrap check.
        if (it->nValue < 0)
