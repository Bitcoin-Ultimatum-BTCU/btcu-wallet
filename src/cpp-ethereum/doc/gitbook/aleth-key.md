...
`cpp-ethereum-key` is a CLI tool that allows you to interact with the Ethereum wallet. With it you can list, inspect, create, delete and modify keys and inspect, create and sign transactions.
...
To create a wallet, run `cpp-ethereum-key` with the `createwallet` command:
...
Occasionally, however, it's important to convert between the two. `cpp-ethereum-key` provides the `inspect` command for this purpose. When passed any address, file or UUID, it will tell you information about it including both formats of address.

For example, to get it to tell us about our account, we might use:

```
> cpp-ethereum-key inspect test
test (0092e965…)
  ICAP: XE472EVKU3CGMJF2YQ0J9RO1Y90BC0LDFZ
  Raw hex: 0092e965928626f8880629cec353d3fd7ca5974f
```
...
```
> cpp-ethereum-key inspect XE472EVKU3CGMJF2YQ0J9RO1Y90BC0LDFZ
test (0092e965…)
  ICAP: XE472EVKU3CGMJF2YQ0J9RO1Y90BC0LDFZ
  Raw hex: 0092e965928626f8880629cec353d3fd7ca5974f
> cpp-ethereum-key inspect 0092e965928626f8880629cec353d3fd7ca5974f
test (0092e965…)
  ICAP: XE472EVKU3CGMJF2YQ0J9RO1Y90BC0LDFZ
  Raw hex: 0092e965928626f8880629cec353d3fd7ca5974f
```
...
On its own, it won't do much. We can see this by decoding it in `cpp-ethereum-key`:

```
> cpp-ethereum-key decode ec80850ba43b74008252089400be78bf8a425471eca0cf1d255118bc080abf95880de0b6b3a7640000801b8080
...
`cpp-ethereum-key` can be used to sign a pre-existing, but unsigned, transaction (it can also create a transaction and sign it itself). In this case, the transaction is actually harmless anyway since we're signing with the key of a fresh account that has no Ether to be transferred.
...
```
> cpp-ethereum-key sign test ec80850ba43b74008252089400be78bf8a425471eca0cf1d255118bc080abf95880de0b6b3a7640000801b8080
...
```
> cpp-ethereum-key decode f86c80850ba43b74008252089400be78bf8a425471eca0cf1d255118bc080abf95880de0b6b3a7640000801ca07638c34170f3e04313bbb6c5bfc10a0c665200515a1aa5e922c7ae6c0dd085faa079ab46048e643bb4042bcb22da86d2646eb0b727f23aa3e165102b824563c70d
...
```
> cpp-ethereum-key import ~/backup-keys/* test
```

or, for Windows:

```
> cpp-ethereum-key import $HOME/backup-keys/*.* test
```
...
If we assume we have a geth key at `mygeth-key.json`, then to import it to use `cpp-ethereum`, simply use:

```
> cpp-ethereum-key import mygeth-key.json "My Old Geth Key"
```
...
Security people reckon that it is prudent to change your password regularly. You can do so easily with `cpp-ethereum-key` using the `recode` command (which actually does a whole lot more, but that's advanced usage).

To do so, simply pass in the name(s) of any keys whose passwords you wish to change. Let's change our key's password:

```
> cpp-ethereum-key recode test
...
There's much more to discover with `cpp-ethereum-key`; it provides a suite of commands for playing with "bare" secrets, those not in the wallet (the `listbare`, `newbare`, ... commands), and allows keys to be imported without actually ever being decrypted (`importwithaddress`) and conversion between ICAP and hex (`inspectbare`).
...
See `cpp-ethereum-key --help` for more information. Enjoy!
