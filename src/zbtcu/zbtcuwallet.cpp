...
CzBTCUWallet::CzBTCUWallet(std::string strWalletFile)
{
    ...
    //Check for old db version of storing zbtcu seed
    if (fFirstRun) {
        uint256 seed;
        if (walletdb.ReadZBTCUSeed_deprecated(seed)) {
            //Update to new format, erase old
            seedMaster = seed;
            hashSeed = Hash(seed.begin(), seed.end());
            if (pwalletMain->AddDeterministicSeed(seed)) {
                if (walletdb.EraseZBTCUSeed_deprecated()) {
                    LogPrintf("%s: Updated zBTCU seed databasing\n", __func__);
                    fFirstRun = false;
                } else {
                    LogPrintf("%s: failed to remove old zbtcu seed\n", __func__);
                }
            }
        }
    }
    ...
    if (fFirstRun) {
        ...
        seedMaster = seed;
        LogPrintf("%s: first run of zbtcu wallet detected, new seed generated. Seedhash=%s\n", __func__, Hash(seed.begin(), seed.end()).GetHex());
...
bool CzBTCUWallet::SetMasterSeed(const uint256& seedMaster, bool fResetCount)
{
    ...
    if (fResetCount)
        walletdb.WriteZBTCUCount(nCountLastUsed);
    else if (!walletdb.ReadZBTCUCount(nCountLastUsed))
        nCountLastUsed = 0;
...
void CzBTCUWallet::Lock()
{
    seedMaster = 0;
}

void CzBTCUWallet::AddToMintPool(const std::pair<uint256, uint32_t>& pMint, bool fVerbose)
{
    mintPool.Add(pMint, fVerbose);
}

//Add the next 20 mints to the mint pool
void CzBTCUWallet::GenerateMintPool(uint32_t nCountStart, uint32_t nCountEnd)
{
    ...
    for (uint32_t i = n; i < nStop; ++i) {
        ...
        CKey key;
        SeedToZBTCU(seedZerocoin, bnValue, bnSerial, bnRandomness, key);
...
bool CzBTCUWallet::LoadMintPoolFromDB()
{
...
void CzBTCUWallet::RemoveMintsFromPool(const std::vector<uint256>& vPubcoinHashes)
{
    for (const uint256& hash : vPubcoinHashes)
        mintPool.Remove(hash);
}

void CzBTCUWallet::GetState(int& nCount, int& nLastGenerated)
{
...
void CzBTCUWallet::SyncWithChain(bool fGenerateMintPool)
{
    ...
    while (found) {
        ...
        for (std::pair<uint256, uint32_t> pMint : listMints) {
            ...
            if (pwalletMain->zbtcuTracker->HasPubcoinHash(pMint.first)) {
...
bool CzBTCUWallet::SetMintSeen(const CBigNum& bnValue, const int& nHeight, const uint256& txid, const libzerocoin::CoinDenomination& denom)
{
    ...
    CBigNum bnRandomness;
    CKey key;
    SeedToZBTCU(seedZerocoin, bnValueGen, bnSerial, bnRandomness, key);
    ...
    // Add to zbtcuTracker which also adds to database
    pwalletMain->zbtcuTracker->Add(dMint, true);
...
void CzBTCUWallet::SeedToZBTCU(const uint512& seedZerocoin, CBigNum& bnValue, CBigNum& bnSerial, CBigNum& bnRandomness, CKey& key)
{
...
uint512 CzBTCUWallet::GetZerocoinSeed(uint32_t n)
{
...
void CzBTCUWallet::GenerateDeterministicZBTCU(libzerocoin::CoinDenomination denom, libzerocoin::PrivateCoin& coin, CDeterministicMint& dMint, bool fGenerateOnly)
{
...
void CzBTCUWallet::GenerateMint(const uint32_t& nCount, const libzerocoin::CoinDenomination denom, libzerocoin::PrivateCoin& coin, CDeterministicMint& dMint)
{
...
bool CzBTCUWallet::CheckSeed(const CDeterministicMint& dMint)
{
...
bool CzBTCUWallet::RegenerateMint(const CDeterministicMint& dMint, CZerocoinMint& mint)
{
    if (!CheckSeed(dMint)) {
        uint256 hashSeed = Hash(seedMaster.begin(), seedMaster.end());
