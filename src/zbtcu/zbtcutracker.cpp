...
CzBTCUTracker::CzBTCUTracker(std::string strWalletFile)
{
...
CzBTCUTracker::~CzBTCUTracker()
{
    mapSerialHashes.clear();
    mapPendingSpends.clear();
}

void CzBTCUTracker::Init()
{
...
bool CzBTCUTracker::Archive(CMintMeta& meta)
{
...
bool CzBTCUTracker::UnArchive(const uint256& hashPubcoin, bool isDeterministic)
{
    CWalletDB walletdb(strWalletFile);
    if (isDeterministic) {
        CDeterministicMint dMint;
...
CMintMeta CzBTCUTracker::Get(const uint256 &hashSerial)
{
...
CMintMeta CzBTCUTracker::GetMetaFromPubcoin(const uint256& hashPubcoin)
{
...
bool CzBTCUTracker::GetMetaFromStakeHash(const uint256& hashStake, CMintMeta& meta) const
{
    for (auto& it : mapSerialHashes) {
...
std::vector<uint256> CzBTCUTracker::GetSerialHashes()
{
...
CAmount CzBTCUTracker::GetBalance(bool fConfirmedOnly, bool fUnconfirmedOnly) const
{
    CAmount nTotal = 0;
...
CAmount CzBTCUTracker::GetUnconfirmedBalance() const
{
    return GetBalance(false, true);
}

std::vector<CMintMeta> CzBTCUTracker::GetMints(bool fConfirmedOnly) const
{
...
bool CzBTCUTracker::HasMintTx(const uint256& txid)
{
    for (auto it : mapSerialHashes) {
...
bool CzBTCUTracker::HasPubcoin(const CBigNum &bnValue) const
{
...
bool CzBTCUTracker::HasPubcoinHash(const uint256& hashPubcoin) const
{
    for (auto it : mapSerialHashes) {
        CMintMeta meta = it.second;
        if (meta.hashPubcoin == hashPubcoin)
...
bool CzBTCUTracker::HasSerial(const CBigNum& bnSerial) const
{
    uint256 hash = GetSerialHash(bnSerial);
    return HasSerialHash(hash);
}

bool CzBTCUTracker::HasSerialHash(const uint256& hashSerial) const
{
    auto it = mapSerialHashes.find(hashSerial);
    return it != mapSerialHashes.end();
}

bool CzBTCUTracker::UpdateZerocoinMint(const CZerocoinMint& mint)
{
...
bool CzBTCUTracker::UpdateState(const CMintMeta& meta)
{
    CWalletDB walletdb(strWalletFile);
...
void CzBTCUTracker::Add(const CDeterministicMint& dMint, bool isNew, bool isArchived, CzBTCUWallet* zBTCUWallet)
{
    bool iszBTCUWalletInitialized = (NULL != zBTCUWallet);
    CMintMeta meta;
    ...
    if (! iszBTCUWalletInitialized)
        zBTCUWallet = new CzBTCUWallet(strWalletFile);
    meta.isSeedCorrect = zBTCUWallet->CheckSeed(dMint);
    if (! iszBTCUWalletInitialized)
        delete zBTCUWallet;
    mapSerialHashes[meta.hashSerial] = meta;
...
void CzBTCUTracker::Add(const CZerocoinMint& mint, bool isNew, bool isArchived)
{
    CMintMeta meta;
...
void CzBTCUTracker::SetPubcoinUsed(const uint256& hashPubcoin, const uint256& txid)
{
...
void CzBTCUTracker::SetPubcoinNotUsed(const uint256& hashPubcoin)
{
    if (!HasPubcoinHash(hashPubcoin))
        return;
    CMintMeta meta = GetMetaFromPubcoin(hashPubcoin);
...
void CzBTCUTracker::RemovePending(const uint256& txid)
{
    uint256 hashSerial;
...
bool CzBTCUTracker::UpdateStatusInternal(const std::set<uint256>& setMempool, CMintMeta& mint)
{
...
std::set<CMintMeta> CzBTCUTracker::ListMints(bool fUnusedOnly, bool fMatureOnly, bool fUpdateStatus, bool fWrongSeed, bool fExcludeV1)
{
    CWalletDB walletdb(strWalletFile);
    if (fUpdateStatus) {
        ...
        CzBTCUWallet* zBTCUWallet = new CzBTCUWallet(strWalletFile);
        for (auto& dMint : listDeterministicDB) {
            if (fExcludeV1 && dMint.GetVersion() < 2)
                continue;
            Add(dMint, false, false, zBTCUWallet);
        }
        delete zBTCUWallet;
        LogPrint("zero", "%s: added %d dzbtcu from DB\n", __func__, listDeterministicDB.size());
    }
