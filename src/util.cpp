...
bool LogAcceptCategory(const char* category)
{
    if (category != NULL) {
        ...
        if (ptrCategory.get() == NULL) {
            ...
            // "btcu" is a composite category enabling all BTCU-related debug output
            if (ptrCategory->count(std::string("btcu"))) {
...
// Windows < Vista: C:\Documents and Settings\Username\Application Data\BTCU
// Windows >= Vista: C:\Users\Username\AppData\Roaming\BTCU
// Mac: ~/Library/Application Support/BTCU
// Unix: ~/.btcu
#ifdef WIN32
    // Windows
...
{
    boost::filesystem::path pathConfigFile(GetArg("-conf", "btcu.conf"));
    ...
    std::map<std::string, std::vector<std::string> >& mapMultiSettingsRet)
{
    ...
    if (!streamConfig.good()) {
        // Create empty btcu.conf if it does not exist
    ...
    for (boost::program_options::detail::config_file_iterator it(streamConfig, setOptions), end; it != end; ++it) {
        // Don't overwrite existing settings so command line settings override btcu.conf
...
{
    boost::filesystem::path pathPidFile(GetArg("-pid", "btcud.pid"));
