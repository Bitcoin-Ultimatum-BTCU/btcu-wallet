...
BOOST_AUTO_TEST_CASE(zerocoin_spend_test)
{
    ...
    cWallet.zbtcuTracker = std::unique_ptr<CzBTCUTracker>(new CzBTCUTracker(cWallet.strWalletFile));
    ...
    BOOST_CHECK_MESSAGE(receipt2.GetStatus() == ZBTCU_TRX_FUNDS_PROBLEMS, strprintf("Failed Invalid Amount Check: %s", receipt.GetStatusMessage()));
...
BOOST_AUTO_TEST_CASE(zerocoin_public_spend_test)
{
    ...
    // -----------------------------------
    int spendVersion = 3;
    BOOST_CHECK_MESSAGE(ZBTCUModule::createInput(in1, mint_v2, tx1.GetHash(), spendVersion),
    ...
    // -----------------------------------
    spendVersion = 4;
    BOOST_CHECK_MESSAGE(ZBTCUModule::createInput(in2, mint_v2, tx2.GetHash(), spendVersion),
