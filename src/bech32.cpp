...
void CBech32Data::Clear()
{
    nVersion = 255;
    vchData.clear();
}

void CBech32Data::SetData(int version, const void* pdata, size_t nSize)
{
    nVersion = version;
    vchData.clear();
    vchData.reserve(54);
    vchData.push_back(nVersion);
    auto begin = static_cast<const unsigned char*>(pdata);
    auto end = begin + nSize;
    ConvertBits<8, 5, true>([&](unsigned char c) { vchData.push_back(c); }, begin, end);
}

bool CBech32Data::SetString(const std::string& str)
{
    Clear();
    ...
    int nTmpVersion = bech.second[0]; // The first 5 bit symbol is the witness version (0-16)
    std::vector<unsigned char> vchTemp;
    vchTemp.reserve(((bech.second.size() - 1) * 5) / 8);

    if (!ConvertBits<5, 8, false>([&](unsigned char c) { vchTemp.push_back(c); }, bech.second.begin() + 1, bech.second.end())) {
        return false;
    }
